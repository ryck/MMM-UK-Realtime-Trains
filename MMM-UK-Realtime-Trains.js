...
Module.register("MMM-UK-Realtime-Trains", {
  ...
  updateTrainsInfo: function (self) {
    self.sendSocketNotification("GET_RTT_DATA", {
      url: self.url,
      config: {
  ...
  getDom: function () {
    ...
    if (this.trains.data !== null) {
      ...
      for (var t in this.trains.data) {
        ...
        expectedDepartureCell.innerHTML = `${moment(
        ...
        expectedArrivalCell.innerHTML = `${moment(
        ...
        var duration = moment
          .duration(moment(train.expectedDeparture, "hmm").diff(moment()))
          .asMinutes();
        Log.info(duration);
